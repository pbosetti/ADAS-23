...
```{r}
# Calcolo i quantili teorici e campionari di 0.25 e 0.75
ql <- tibble(
  p = c(1,3)/4,
  sq = quantile(df$x, p),
  tq = qnorm(p)
)
# Equazione della retta y=ax+b: a=(y2-y1)/(x2-x1), b=y1-ax1
a <- with(ql, (sq[2] - sq[1]) / (tq[2] - tq[1]))
b <- with(ql, sq[1] - a * tq[1])
# Aggungo al grafico i due punti corrispondenti ai quantili e la retta passante
plot +
  geom_point(aes(x=tq, y=sq), ql, shape=4, size=4, stroke=2, color="red") +
  geom_abline(aes(slope=a, intercept=b), color="red")

# Confronto con le funzioni geom_qq()
...
**Soluzione**:
Uso `attributes(df.tuk)` per scoprire il contenuto dell'oggetto. Il campo `$Cotton` è una matrice che posso utilizzare per il grafico. Le etichette delle differenze sono disponibili come nomi di riga: posso estrarne il vettore con `rownames(df.tuk$Cotton)`.
Inoltre, se vogliamo che il grafico rispetti lo stesso ordine delle coppie come nel plot originale, dobbiamo definire un campo `pair` come fattore ordinato, usando come ordine dei livelli lo stesso restituito da `rownames(df.tuk$Cotton)` (in caso contrario, ggplot riordina le coordinate qualitative secondo l'ordine alfabetico).

```{r}
df.tuk$Cotton %>% 
  as_tibble(rownames=NA) %>% # rownames=NA significa "mantieni i numeri di riga"
  mutate(
    # ricordare che . significa l'oggetto ricevuto dalla pipe
    pair=factor(rownames(.), ordered = T, levels=rev(rownames(.))), 
    reject=`p adj` < 0.01, # nomi di colonne con spazio vanno messi tra backtick
  ) %>% 
  ggplot(aes(x=pair, y=diff, color=reject)) +
